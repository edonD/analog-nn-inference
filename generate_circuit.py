...
DEFAULT_PARAMS = {
    ...
    "diode_is": 1e-15,     # Diode saturation current
    "diode_n": 0.001,      # Diode ideality factor (lower = sharper ReLU)
    ...
    if mode == "crossbar":
        ...
        for j in range(n_hidden):
            ...
            for i in range(n_in):
                ...
                r_plus = max(R_min, min(1.0 / g_plus, R_max))
                r_minus = max(R_min, min(1.0 / g_minus, R_max))
...
def main():
    ...
    parser.add_argument("--pixel-values", type=str, default=None,
                        help="Comma-separated 64 pixel values (0-1)")
    ...
    if args.pixel_values:
        ...
        if len(pixels) != 64:
            print(f"ERROR: Expected 64 pixel values, got {len(pixels)}")
            sys.exit(1)
        label = args.label
